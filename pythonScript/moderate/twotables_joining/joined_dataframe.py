...
# 创建SparkSession
spark = SparkSession.builder.appName("2Tables_joined_Dataframe").getOrCreate()
...
def create_uneven_dataframe(num_rows, num_columns):
    ...
    # 设置主要日期范围和少数日期范围
    main_date_start = datetime(2024, 6, 1)
    main_date_end = datetime(2024, 6, 18)  # 集中在6月
...
def joined_dataframe(df1,df2):
  df = df1.join(df2, on='business_date', how='inner')
  return df


df_joined = joined_dataframe(df_uneven,df_even)
df_joined.show()
